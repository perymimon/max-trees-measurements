...
export function usePrevious(value, initialValue, refValue) {
...
export function useChangeIntersection(tracking, options = {}) {
    ...
    const currentArray = [tracking].flat(1) // convert tracking to array
    const prevArray = usePrevious(currentArray, [], tracking);

    // if key not provided, use the keyValue at that position as key
    const getKey = (item, i) =>
        typeof key === 'function' ? key(item, i) :
            key == 'index' ? i :
                key === 'generate' ? keyGenerator(item, i) :
                    item[key] ?? item
    ...
    return useMemo(_ => {
        ...
        for (let [i, item] of prevArray.entries()) {
            ...
            state = {
                key: k, to: Infinity, ...state, item, phase, from: i,
            }
            unionMap.set(k, state)
            if (phase === REMOVE) exportOrder.splice(i, 0, state)
        }
